...
public:
	...
	[[nodiscard]] unsigned int get_code_size() const;

	void add_variable(Variable* variable);
	std::vector<Variable*>& get_procedure_variables();
	std::vector<Variable*>& get_procedure_pointers();

	[[nodiscard]] unsigned int get_procedure_start_point() const;
	void set_procedure_start_point(unsigned int start_point);

	[[nodiscard]] unsigned int get_procedure_ending_jump_position() const;
...
private:
	...
	unsigned int m_ending_jump_position = 0;
